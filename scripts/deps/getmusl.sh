...
*cpp_options:
-nostdinc -isystem ${incdir} -isystem /usr/include/linux -isystem include%s %(old_cpp_options)

*cc1:
%(cc1_cpu) -nostdinc -isystem ${incdir} -isystem /usr/include/linux -isystem include%s
...
EOF
echo "${ROOT}/musl/bin/musl-gcc" > ${DIRNAME}/musl-gcc-path

rm -rf ${ROOT}/musl-src
rm ${MUSL}-${VERSION}.tar.gz

# prepare linux headers
cp -rs /usr/include/linux ${incdir}
cp -rs /usr/include/x86_64-linux-gnu/asm ${incdir}/asm
cp -rs /usr/include/asm-generic ${incdir}/asm-generic
