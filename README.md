...
```shell
 ./scripts/getmusl.sh <absolute-path-to-NoTamper>
```
...
make load
```
Then the kernel module is loaded. You can find the kernel module file called `notamper.ko` in project root directory.

### Configuration
In NoTamper, there are 3 variables can be configured with cmake.

- `BUFFER_SIZE`: the size of each per-thread buffer (default value: 8KB)
- `CONSUMER_PATH`: the path to find monitor executable (default value: `${PROJECT_BINARY_PATH}/consumer/consumer`)
- `STORE_PATH`: the pare that store the event data (default value: `/tmp/notamper`)

When you generate cmake files, you can specify these variables. For example, if you want to set that buffer size is 4MB, consumer path is `/my/path/to/consumer` and store path is `/my/path/to/store`, you can run the following commands
...
NoTamper also supports customed log path format. In default, the log path format in C-like is `STORE_PATH/%u-%ld.buf`, which accepts two arguments including thread id and ns-scale timestamp. This log path format can also be configured with CMake by defining a macro. For example, if you would like to specify the log path format to a tty device, the following commands will achieve this.
...
### Pkey Support
NoTamper utilizes Intel Protection Key (PKEY) to protect its memory. If your machine does not support pkey, you must disable it otherwise a SIGILL will triggered due to the illegel instruction used by pkey.
